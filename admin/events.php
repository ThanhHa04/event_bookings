...
// Lấy dữ liệu sự kiện theo phân trang
$query = "SELECT * FROM events WHERE $where ORDER BY start_time DESC LIMIT $limit OFFSET $offset";
$stmt = $pdo->prepare($query);
$stmt->execute($params);
$events = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Lấy danh sách sự kiện đã có ghế đặt
...
$nextEventId = 'E0' . str_pad((int)$maxId + 1, 4, '0', STR_PAD_LEFT);
...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-3">
        ...
        <form class="d-flex align-items-center gap-2" method="GET" action="" style="flex: 1; justify-content: flex-end;">
            ...
            <input type="date" class="form-control flex-shrink-0" name="filter_date"
                value="<?= htmlspecialchars($filter_date) ?>" style="max-width: 150px;">
            <input type="text" class="form-control" name="search" style="max-width: 300px;"
                placeholder="Tìm kiếm tên sự kiện" value="<?= htmlspecialchars($search) ?>">
            <button class="btn btn-outline-primary flex-shrink-0" type="submit"><i class="bi bi-search"></i></button>
            <button type="button" class="btn btn-success flex-shrink-0" id="createEventBtn"
            ...
            <ul class="pagination justify-content-center">
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <li class="page-item <?= $i == $page ? 'active' : '' ?> ">
                        <a class="page-link" href="?status=<?= htmlspecialchars($status) ?>&page=<?= $i ?>&search=<?= urlencode($search) ?>&filter_date=<?= urlencode($filter_date) ?>"><?= $i ?></a>
                    </li>
                <?php endfor ?>
            </ul>
        </nav>
