...
if (!isset($_GET["event_id"]) || !is_numeric($_GET["event_id"])) {
    echo "Lỗi: Không tìm thấy sự kiện nổi bật.";
    exit();
}

$event_id = intval($_GET["event_id"]);
$stmt = $pdo->prepare("SELECT * FROM featured_events WHERE id = ?");
$stmt->execute([$event_id]);
$event = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$event) {
    echo "Lỗi: Sự kiện nổi bật không tồn tại.";
    exit();
}
?>
...
<div class="container mt-3 mb-3">
    <div class="event-image mb-4 text-center">
        <img src="<?php echo (str_starts_with($event['image'], 'http') ? $event['image'] : '../assets/images/' . htmlspecialchars($event['image'])); ?>" class="img-fluid rounded" alt="Featured Event Image">
    </div>
    ...
    <div class="event-info p-4" style="box-shadow: 0 4px 12px gray; border-radius: 20px;">
        ...
        <p><i class="bi bi-cash" style="color: #ff5722;"></i> <?php echo number_format($event["price"], 0, ",", "."); ?> đ</p>
